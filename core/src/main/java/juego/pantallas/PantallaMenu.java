...
private float volumeLevel = 0.5f; // Nivel de volumen (0.0 a 1.0)
    ...
    private Rectangle volumeSliderRect = new Rectangle();
    private Rectangle volumeHandleRect = new Rectangle();
   ...
   private void loadFont() {
    ...
    private void loadBackgrounds() {
         if (Gdx.files.internal("fondos/fondo.png").exists()) {
            ...
            Gdx.app.log("PantallaMenu", "Fondo principalcargado correctamente");
        } else {
        ...
        if (Gdx.files.internal("fondos/fondoOpciones.png").exists()) {
            ...
            Gdx.app.error("PantallaMenu", "No se encontrófondos/fondoOpciones.png,usandofondo procedural para opciones");
        }
    }
...
if (font != null) {
    ...
    private void drawVolumeSlider(SpriteBatch batch) {
        com.badlogic.gdx.math.Vector2 mouse = viewport.unproject(new com.badlogic.gdx.math.Vector2(Gdx.input.getX(), Gdx.input.getY()));
       boolean hovered = volumeSliderRect.contains(mouse.x, mouse.y) || volumeHandleRect.contains(mouse.x,mouse.y);

        // Dibujar fondo del slider
        batch.setColor(new Color(0, 0, 0, 0.5f));
        batch.draw(getWhitePixel(), volumeSliderRect.x, volumeSliderRect.y, volumeSliderRect.width, volumeSliderRect.height);

        //Dibujar barra de volumen
        batch.setColor(hovered ? Color.valueOf("F0C850") : Color.GOLD);
        batch.draw(getWhitePixel(), volumeSliderRect.x, volumeSliderRect.y, volumeSliderRect.width *volumeLevel, volumeSliderRect.height);

// Dibujar handlebatch.setColor(hovered ? Color.valueOf("F0C850") : Color.WHITE);
        batch.draw(getWhitePixel(), volumeHandleRect.x, volumeHandleRect.y, volumeHandleRect.width, volumeHandleRect.height);

        //Dibujar etiqueta
if(font != null) {
           font.setColor(hovered ? Color.valueOf("F0C850") : Color.WHITE);
            font.getData().setScale(1f);
            font.draw(batch, "Volumen", volumeSliderRect.x, volumeSliderRect.y + volumeSliderRect.height + 25);
}
...
if (!inOptionsMode) {
               ...
               if (btnCloseOptionsRect.contains(x, y)) toggleOptions();
                    ...
                    handleVolumeInput(x, y);
                }
            }
        }
        
        // Manejar arrastre del slider
        if (Gdx.input.isTouched() && inOptionsMode) {
            com.badlogic.gdx.math.Vector2 touch= viewport.unproject(new com.badlogic.gdx.math.Vector2(Gdx.input.getX(), Gdx.input.getY()));
            handleVolumeInput(touch.x, touch.y);
        }
    }

    private void handleVolumeInput(float x, float y) {
        // Verificar si se hizo clic o se está arrastrando el slider
if (volumeSliderRect.contains(x, y)|| volumeHandleRect.contains(x, y)) {
            // Calcular el nuevo valor de volumen basado en la posición del clic
            float relativeX = x - volumeSliderRect.x;
            volumeLevel = Math.max(0f, Math.min(1f, relativeX / volumeSliderRect.width));
            
// Actualizar posición del handle
            volumeHandleRect.x = volumeSliderRect.x + (volumeSliderRect.width * volumeLevel) - volumeHandleRect.width/2;
            
            // Aplicar volumen a las canciones
            updateMusicVolume();
}
    }
       ...
       canciones = new Music[rutasCanciones.length];
        for (int i = 0; i < rutasCanciones.length; i++) {
            if (Gdx.files.internal(rutasCanciones[i]).exists()) {
                ...
                canciones[i].setVolume(volumeLevel); // Usar el nivel de volumen actual
                canciones[i].play();
            }
        }
    }
